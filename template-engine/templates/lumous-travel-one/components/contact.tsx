...
function Lines({ label, lines }: { label: unknown; lines: string[] }) {
  return (
    <div className="flex flex-col gap-2">
      <span className="font-lt-label text-lt-subtle-foreground text-[12.5px] font-extralight tracking-[0.14em] uppercase">
        {trimmed(label)}
      </span>
      <span>
  ...
  return (
    ...
    >
      ...
      <div className="grid [grid-template-columns:repeat(auto-fit,minmax(min(100%,320px),1fr))] items-end gap-[clamp(32px,5vw,72px)]">
        <h2 className="font-lt-display text-lt-display-lg tracking-lt-tight text-lt-heading m-0 leading-[1.1] font-extralight text-pretty">
        ...
        <div className="text-lt-sm text-lt-muted-foreground grid [grid-template-columns:repeat(auto-fit,minmax(160px,1fr))] gap-[clamp(20px,3vw,36px)] leading-[1.8]">
